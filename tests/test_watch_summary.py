...
def stub_summary_dependencies(monkeypatch: pytest.MonkeyPatch, bundle: SummaryBundle, payload: dict) -> None:
    ...
    monkeypatch.setattr(
        "watch_cheeks.load_masked_history",
        lambda: {"mask_level": 1, "days": {}},
    )
    monkeypatch.setattr(
        "watch_cheeks.public_summary.build_summary_context",
        lambda period_key, dataset, history_meta, masking_config=None: object(),
    )
    monkeypatch.setattr(
        "watch_cheeks.public_summary.build_slack_payload",
        lambda context, title, logical_today=None: (payload, "fallback", [("sec", ["line"])]),
    )
...
def test_summary_writes_nested_raw_output(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert content["window_days"] == 7
    assert "logical_today" in content


def test_summary_uses_placeholder_when_context_is_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    bundle = make_bundle()
    stub_summary_dependencies(monkeypatch, bundle, {"text": "ignored"})
    monkeypatch.setattr(
        "watch_cheeks.public_summary.build_summary_context",
        lambda period_key, dataset, history_meta, masking_config=None: None,
    )
    monkeypatch.setattr(
        "watch_cheeks.public_summary.build_placeholder_summary_payload",
        lambda title, message: ({"text": "placeholder"}, "placeholder", [("sec", ["line"])]),
    )
