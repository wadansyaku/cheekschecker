...
Band = Tuple[Union[int, float], Optional[Union[int, float]], str]
...
class MaskingConfig:
    ...
    count_bands: Tuple[Band, ...]
    total_bands: Tuple[Band, ...]
    ratio_bands: Tuple[Band, ...]
    level2_words: dict[str, Tuple[str, ...]]
    level2_divisors: dict[str, int]
    level2_ratio_thresholds: Tuple[float, ...]


def _parse_band_sequence(raw: Iterable, *, value_cast) -> Tuple[Band, ...]:
    bands: list[Band] = []
    for item in raw:
...
def _parse_level2_words(raw: dict[str, Sequence[str]]) -> dict[str, Tuple[str, ...]]:
...
def _parse_level2_divisors(raw: dict[str, object]) -> dict[str, int]:
    parsed: dict[str, int] = {}
...
def _parse_ratio_thresholds(raw: Sequence[object]) -> Tuple[float, ...]:
